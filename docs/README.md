# 📚 Documentation Index

Welcome to the comprehensive documentation for the Next.js ### 🔧 Utilities & Scripts

- [**Scripts Documentation**](../scripts/README.md) - Available utility scripts
- [**Password Hashing**](../scripts/utils/hash-password.js) - Password utility
- [**Vertex AI Diagnostics**](../scripts/utils/diagnose-vertex-ai.sh) - AI service testing

### 🤖 AI Integration

- [**GitHub Copilot Instructions**](../.github/copilot-instructions.md) - AI pair programming context
- [**Chat Modes Documentation**](../.github/chatmodes/README.md) - Specialized AI assistance modes
- [**Security Scout Mode**](../.github/chatmodes/security-scout.chatmode.md) - Security-focused code reviewApplication. This index provides a structured guide to all available documentation.

## 🚀 Quick Start

| Document                                                      | Description                      | Audience   |
| ------------------------------------------------------------- | -------------------------------- | ---------- |
| [README.md](../README.md)                                     | Project overview and quick setup | Everyone   |
| [Getting Started](DEVELOPMENT.md#initial-setup)               | Detailed setup instructions      | Developers |
| [Environment Setup](DEVELOPMENT.md#environment-configuration) | Configure environment variables  | Developers |

## 👩‍💻 Developer Documentation

### Core Development

- [**Development Guide**](DEVELOPMENT.md) - Complete development setup and workflows
- [**API Documentation**](API.md) - REST API endpoints and usage
- [**Contributing Guidelines**](../CONTRIBUTING.md) - How to contribute to the project
- [**Code of Conduct**](../CODE_OF_CONDUCT.md) - Community standards and behavior

### Architecture & Design

- [**GitHub Copilot Instructions**](../.github/copilot-instructions.md) - AI pair programming context
- [**Performance Guide**](PERFORMANCE.md) - Optimization strategies and monitoring
- [**User Management**](USER-MANAGEMENT.md) - Authentication and user handling

### Features & Implementation

- [**Feature Flags**](features/FEATURE-FLAGS.md) - Feature toggle system
- [**Middleware Security**](features/MIDDLEWARE-SECURITY-SUMMARY.md) - Security implementation details
- [**Model Selection**](features/MODEL-SELECTION.md) - AI model configuration
- [**Database Integration**](features/DATABASE-INTEGRATION.md) - Database setup and usage

## 🧪 Testing Documentation

- [**E2E Testing Guide**](testing/E2E-TESTING-GUIDE.md) - End-to-end testing with Playwright
- [**E2E Testing Summary**](testing/E2E-TESTING-SUMMARY.md) - Testing strategy overview
- [**E2E Testing Optimizations**](testing/E2E-TESTING-OPTIMIZATIONS.md) - Performance improvements

## 🚀 Deployment & Operations

### Deployment Guides

- [**Deployment Overview**](deployment/DEPLOY.md) - General deployment information
- [**Cloud Run Deployment**](deployment/CLOUD-RUN-DEPLOYMENT.md) - Google Cloud Run specific guide
- [**Manual Deploy Commands**](deployment/MANUAL-DEPLOY-COMMANDS.md) - Command-line deployment

### CI/CD & Automation

- [**CI/CD Pipeline**](deployment/CI-CD.md) - Continuous integration and deployment
- [**GitHub Actions Setup**](deployment/GITHUB-ACTIONS-SETUP.md) - Automated workflows configuration
- [**GitHub Actions Deployment**](deployment/GITHUB-ACTIONS-DEPLOYMENT.md) - Automated deployment setup
- [**Workflows Explained**](deployment/WORKFLOWS-EXPLAINED.md) - Detailed workflow documentation

### Deployment Checklists

- [**Deployment Checklist**](deployment/DEPLOYMENT-CHECKLIST.md) - Pre-deployment verification
- [**GitHub Actions Status**](deployment/GITHUB-ACTIONS-STATUS.md) - Current automation status

## 🔄 Migration & Updates

- [**Gemini 2.0 Migration**](migration/GEMINI-2.0-MIGRATION.md) - AI model upgrade guide
- [**Gemini 2.5 Implementation**](migration/GEMINI-2.5-IMPLEMENTATION.md) - Latest model integration
- [**Dynamic Model Fetching**](migration/DYNAMIC-MODEL-FETCHING.md) - Runtime model selection
- [**Rate Limiting Migration**](migration/RATE-LIMITING-MIGRATION.md) - Rate limiting system updates

## 📋 Project History & Archive

### Implementation Summaries

- [**Phase 2 Summary**](archive/PHASE2-SUMMARY.md) - Major development milestone
- [**Phase 3 Summary**](archive/PHASE3-SUMMARY.md) - Advanced features implementation
- [**Improvements Summary**](archive/IMPROVEMENTS.md) - Comprehensive improvement log

### Specialized Features

- [**Image Generation**](features/image-generation/) - AI image generation capabilities
- [**Dynamic Model Implementation**](migration/DYNAMIC-MODEL-IMPLEMENTATION-SUMMARY.md) - Model selection system

## 🛡️ Security & Compliance

- [**Security Policy**](../SECURITY.md) - Vulnerability reporting and security measures
- [**Middleware Security Summary**](features/MIDDLEWARE-SECURITY-SUMMARY.md) - Security implementation details

## 🔧 Utilities & Scripts

- [**Scripts Documentation**](../scripts/README.md) - Available utility scripts
- [**Password Hashing**](../scripts/utils/hash-password.js) - Password utility
- [**Vertex AI Diagnostics**](../scripts/utils/diagnose-vertex-ai.sh) - AI service testing

## 📊 Documentation Categories

### By Audience

- **🆕 New Contributors**: README → CONTRIBUTING → DEVELOPMENT
- **👩‍💻 Developers**: DEVELOPMENT → API → Testing docs
- **🚀 DevOps**: All deployment/\* documents
- **🤖 GitHub Copilot**: `.github/copilot-instructions.md`
- **🛡️ Security Team**: SECURITY.md → Middleware Security docs

### By Topic

- **🔧 Setup**: README, DEVELOPMENT, Environment guides
- **📡 API**: API.md, Authentication docs
- **🧪 Testing**: All testing/\* documents
- **🚀 Deployment**: All deployment/\* documents
- **🤖 AI/ML**: Model selection, Vertex AI, Migration docs
- **🛡️ Security**: Security policy, Middleware security
- **📈 Performance**: Performance guide, Optimizations

## 🆘 Getting Help

- **📝 Create an Issue**: Use our [issue templates](../.github/ISSUE_TEMPLATE/)
- **💬 Start a Discussion**: [GitHub Discussions](https://github.com/roofsonfire/chat/discussions)
- **🛡️ Report Security Issues**: Follow our [Security Policy](../SECURITY.md)

## 📝 Documentation Standards

This documentation follows these principles:

- **📱 Responsive**: Works on all devices and screen sizes
- **🔍 Searchable**: Well-structured with clear headings
- **🎯 Actionable**: Includes step-by-step instructions
- **🔄 Up-to-date**: Regularly maintained and reviewed
- **🤖 AI-Friendly**: Optimized for GitHub Copilot context

---

**Last Updated**: January 2025  
**Maintained by**: Development Team  
**Review Schedule**: Monthly

> 💡 **Tip**: Use `Ctrl+F` (or `Cmd+F` on Mac) to quickly find specific topics in this index.
