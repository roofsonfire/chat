  ...
  lines.forEach((line, index) => {
    ...
    if (line.match(/^#+[^# ]/) && !line.match(/^#+$/)) {
      errors.push({
        ...
        line: lineNum,
        message:
          "Header missing space after # (e.g., '## Title' not '##Title')",
      });
    }
  });
...
if (results.totalErrors === 0 && results.totalWarnings === 0) {
  ...
  console.log(
    `❌ Tests failed with ${results.totalErrors} error(s) and ${results.totalWarnings} warning(s)\n`
  );
  process.exit(1);
}
