      ...
      contents: [
        {
          ...
          parts: [
            {
              text: "Generate a simple image of a small red heart on white background",
            },
          ],
        },
      ],
    ...
    if (response.candidates && response.candidates[0]) {
      ...
      parts.forEach((part, index) => {
        if (part.text) {
          textParts++;
          console.log(
            `    - Part ${index + 1}: TEXT (${part.text.length} chars)`
          );
        ...
        if (part.inlineData) {
          ...
          console.log(
            `      Data Length: ${part.inlineData.data?.length || 0} chars (base64)`
          );
        }
      });
      ...
      if (imageParts === 0) {
        ...
        console.log(
          '    2. Try a more explicit prompt like "create an image of..."'
        );
      }
    }
  } catch (error) {
