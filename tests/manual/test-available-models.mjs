...
async function main() {
  ...
  if (!process.env.GOOGLE_PROJECT_ID || !process.env.GOOGLE_LOCATION) {
    ...
    console.error(
      "   GOOGLE_PROJECT_ID:",
      process.env.GOOGLE_PROJECT_ID || "Missing"
    );
    console.error(
      "   GOOGLE_LOCATION:",
      process.env.GOOGLE_LOCATION || "Missing"
    );
    process.exit(1);
  }
  ...
  if (available.length > 0) {
    ...
    console.log(
      `   For example: GOOGLE_VERTEX_AI_MODEL_ID=${available[0].model}\n`
    );
  } else {
  ...
  if (unavailable.length > 0) {
    ...
    unavailable.forEach((r) => {
      const errorKey = r.error.includes("404")
        ? "404 Not Found"
  ...
  if (available.length === 0) {
    ...
    console.log(
      "   2. Try a different region (e.g., us-central1, europe-west4)"
    );
    console.log("   3. Verify project access:");
    console.log(
      "      gcloud projects get-iam-policy " + process.env.GOOGLE_PROJECT_ID
    );
